...
namespace lns {
    ...
    public:
        ...
        explicit block_stmt(const char *file, const int line, const vector<stmt *> &statements) : stmt(line, file,
                                                                                                       BLOCK_STMT_T),
                                                                                                  statements(
                                                                                                          statements) {}
    ...
    public:
        ...
        explicit expression_stmt(const char *file, const int line, const expr &exprs) : stmt(line, file,
                                                                                             EXPRESSION_STMT_T),
                                                                                        exprs(exprs) {}
    ...
    public:
        ...
        function_stmt(const char *file, const int line, const token *name, const vector<token *> &parameters,
                      const vector<stmt *> &body, const visibility vis) : stmt(line, file, FUNCTION_STMT_T), name(name),
    ...
    public:
        ...
        context_stmt(const char *file, const int line, const token *name, const vector<stmt *> body,
                     const visibility vis, const bool isfinal) : stmt(line, file, CONTEXT_STMT_T), name(name),
                                                                 body(body), vis(vis), isfinal(isfinal) {}
    ...
    public:
        ...
        if_stmt(const char *file, const int line, const expr &condition, const stmt *thenBranch, const stmt *elseBranch)
                : stmt(line, file, IF_STMT_T), condition(condition), thenBranch(thenBranch), elseBranch(elseBranch) {}
    ...
    public:
        ...
        return_stmt(const char *file, const int line, const token *keyword, const expr &value) : stmt(line, file,
                                                                                                      RETURN_STMT_T),
                                                                                                 keyword(keyword),
                                                                                                 value(value) {}
    ...
    public:
        ...
        explicit break_stmt(const char *file, const int line, const token *keyword) : stmt(line, file, BREAK_STMT_T),
                                                                                      keyword(keyword) {}
    ...
    public:
        ...
        explicit continue_stmt(const char *file, const int line, const token *keyword) : stmt(line, file,
                                                                                              CONTINUE_STMT_T),
                                                                                         keyword(keyword) {}
    ...
    public:
        ...
        var_stmt(const char *file, const int line, const token *name, const expr &initializer, const visibility vis,
                 const bool isfinal) : stmt(line, file, VAR_STMT_T), name(name), initializer(initializer), vis(vis),
                                       isfinal(isfinal) {}
    ...
    public:
        ...
        s_while_stmt(const char *file, const int line, const expr &condition, const stmt *body) : stmt(line, file,
                                                                                                       S_WHILE_STMT_T),
                                                                                                  condition(condition),
                                                                                                  body(body) {}
    ...
    public:
        ...
        s_for_stmt(const char *file, const int line, const stmt *init, const expr *condition, const expr *increment,
                   const stmt *body) : stmt(line, file, S_FOR_STMT_T), init(init), condition(condition),
    ...
    public:
        ...
        s_for_each_stmt(const char *file, const int line, const token *identifier, const expr *container,
                        const stmt *body) : stmt(line, file, S_FOR_EACH_STMT_T), identifier(identifier),
    ...
    public:
        ...
        raise_stmt(const char *file, const int line, const token *where, const expr *name,
                   const map<string, expr *> &fields) : stmt(line, file, RAISE_STMT_T), where(where), name(name),
                                                        fields(fields) {}
    ...
    public:
        ...
        uses_native_stmt(const char *file, const int line, const token *where, const string &lib_name,
                         const token *bind) : stmt(line, file, USES_NATIVE_STMT_T), where(where), lib_name(lib_name),
                                              bind(bind) {}
    ...
    public:
        ...
        handle_stmt(const char *file, const int line, const expr *exception_name, const token *bind,
                    const vector<stmt *> &block) : stmt(line, file, HANDLE_STMT_T), exception_name(exception_name),
    ...
    public:
        ...
        begin_handle_stmt(const char *file, const int line, const vector<stmt *> &body,
                          const vector<handle_stmt *> handles) : stmt(line, file, BEGIN_HANDLE_STMT_T), body(body),
                                                                 handles(handles) {}
    ...
    public:
        ...
        constructor_stmt(const char *file, const int line, const token *keyword, const vector<token *> &parameters,
                         const vector<stmt *> &body, const visibility vis) : stmt(line, file, CONSTRUCTOR_STMT_T),
                                                                             keyword(keyword), parameters(parameters),
                                                                             body(body), vis(vis) {}
    ...
    public:
        ...
        class_decl_stmt(const char *file, const int line, const token *name, const vector<function_stmt *> &methods,
                        ...
                        const visibility vis) : stmt(line, file, CLASS_DECL_STMT_T), name(name), methods(methods),
                                                constructors(constructors), variables(variables), vis(vis) {}
    ...
    public:
        ...
        explicit null_stmt(const char *file, const int line, const token *where) : stmt(line, file, NULL_STMT_T),
                                                                                   where(where) {}

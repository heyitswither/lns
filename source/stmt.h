...
const token* bind;
uses_native_stmt(const char* file,const int line, const token* where, const string& lib_name, const token* bind) : stmt(line, file, USES_NATIVE_STMT_T), where(where), lib_name(lib_name), bind(bind) {}
void accept(stmt_visitor *v) override{

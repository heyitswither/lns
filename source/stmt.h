...
namespace lns {
    ...
    public:
        ...
        explicit block_stmt(const char *file, const int line, const vector<shared_ptr<stmt>> statements) : stmt(line,
                                                                                                                file,
                                                                                                                BLOCK_STMT_T),
                                                                                                           statements(
                                                                                                                   statements) {}
    ...
    public:
        ...
        return_stmt(const char *file, const int line, const token *keyword, const shared_ptr<expr> value) : stmt(line,
                                                                                                                 file,
                                                                                                                 RETURN_STMT_T),
                                                                                                            keyword(keyword),
                                                                                                            value(value) {}
    ...
    public:
        ...
        raise_stmt(const char *file, const int line, const token *where, const shared_ptr<expr> name,
                   const map<string, shared_ptr<expr>> fields) : stmt(line, file, RAISE_STMT_T), where(where),
    ...
    public:
        ...
        handle_stmt(const char *file, const int line, const shared_ptr<expr> exception_name, const token *bind,
                    const vector<shared_ptr<stmt>> block) : stmt(line, file, HANDLE_STMT_T),
                                                            exception_name(exception_name), bind(bind), block(block) {}
    ...
    public:
        ...
        constructor_stmt(const char *file, const int line, const token *keyword, const parameter_declaration parameters,
                         const vector<shared_ptr<stmt>> body, const visibility vis) : stmt(line, file,
                                                                                           CONSTRUCTOR_STMT_T),
                                                                                      keyword(keyword),
    ...
    public:
        ...
        const shared_ptr<expr> super;
        const visibility vis;

        class_decl_stmt(const char *file, const int line, const token *name,
                        const vector<shared_ptr<function_stmt>> methods,
                        const vector<shared_ptr<constructor_stmt>> constructors,
                        const vector<shared_ptr<var_stmt>> variables, const shared_ptr<expr> super,
                        const visibility vis) : stmt(line, file, CLASS_DECL_STMT_T), name(name), methods(methods),
                                                constructors(constructors), variables(variables), super(super),
                                                vis(vis) {}

...
namespace lns {
    enum expr_type {
        ...
        VARIABLE_EXPR_T,
        MAP_FIELD_EXPR_T,
        ASSIGN_MAP_FIELD_EXPR_T,
        CONTEXT_EXPR_T,
        CONTEXT_ASSIGN_EXPR_T,
        NULL_EXPR_T,
    };
    ...
    public:
        increment_expr(const char *file, int line, token &name, expr *value) : name(name), value(value),
                                                                               expr(line, file, INCREMENT_EXPR_T) {}
    ...
    public:
        decrement_expr(const char *file, int line, token &name, expr *value) : name(name), value(value),
                                                                               expr(line, file, DECREMENT_EXPR_T) {}
    ...
    public:
        assign_expr(const char *file, int line, token &name, expr *value) : name(name), value(value),
                                                                            expr(line, file, ASSIGN_EXPR_T) {}
    ...
    public:
        binary_expr(const char *file, int line, expr *left, token &op, expr *right) : left(left), op(op), right(right),
                                                                                      expr(line, file, BINARY_EXPR_T) {}
    ...
    public:
        call_expr(const char *file, int line, expr *callee, token &paren, vector<expr *> &args) : callee(callee),
                                                                                                  paren(paren),
                                                                                                  args(args),
                                                                                                  expr(line, file,
                                                                                                       CALL_EXPR_T) {}
    ...
    public:
        explicit grouping_expr(const char *file, int line, expr *expression) : expression(expression),
                                                                               expr(line, file, GROUPING_EXPR_T) {}
    ...
    public:
        explicit literal_expr(const char *file, int line, object *value) : value(value),
                                                                           expr(line, file, LITERAL_EXPR_T) {}
    ...
    public:
        unary_expr(const char *file, int line, token &op, expr *right) : op(op), right(right),
                                                                         expr(line, file, UNARY_EXPR_T) {}
    ...
    public:
        explicit variable_expr(const char *file, int line, token &name) : name(name),
                                                                          expr(line, file, VARIABLE_EXPR_T) {}
    ...
    public:
        map_field_expr(const char *file, int line, token &name, expr *key) : name(name), key(key),
                                                                             expr(line, file, MAP_FIELD_EXPR_T) {}
    ...
    public:
        assign_map_field_expr(const char *file, int line, token &name, expr *key, expr *value) : name(name), key(key),
                                                                                                 value(value),
                                                                                                 expr(line, file,
                                                                                                      ASSIGN_MAP_FIELD_EXPR_T) {}
    ...
    public:
        context_expr(const char *file, int line, token &context_name, token &context_identifier) : context_name(
                context_name), context_identifier(context_identifier), expr(line, file, CONTEXT_EXPR_T) {}
    ...
    public:
        context_assign_expr(const char *file, int line, token &context_name, token &context_identifier, expr *value)
                : context_name(context_name), context_identifier(context_identifier), value(value),
                  expr(line, file, CONTEXT_ASSIGN_EXPR_T) {}

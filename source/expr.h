...
namespace lns {
    ...
    public:
        ...
        assign_expr(const char *file, const int line, const token *name, const token_type op, const expr *value) : expr(
                line, file, ASSIGN_EXPR_T), name(name), op(op), value(value) {}
    ...
    public:
        ...
        binary_expr(const char *file, const int line, const expr *left, const token *op, const expr *right) : expr(line,
                                                                                                                   file,
                                                                                                                   BINARY_EXPR_T),
                                                                                                              left(left),
                                                                                                              op(op),
    ...
    public:
        ...
        call_expr(const char *file, const int line, const expr *callee, const token *paren, const vector<expr *> &args)
                : expr(line, file, CALL_EXPR_T), callee(callee), paren(paren), args(args) {}
    ...
    public:
        ...
        explicit grouping_expr(const char *file, const int line, const expr *expression) : expr(line, file,
                                                                                                GROUPING_EXPR_T),
    ...
    public:
        ...
        explicit literal_expr(const char *file, const int line, const object *value) : expr(line, file, LITERAL_EXPR_T),
                                                                                       value(value) {}
    ...
    public:
        ...
        unary_expr(const char *file, const int line, const token *op, const operator_location location,
                   const expr *right) : expr(line, file, UNARY_EXPR_T), op(op), location(location), right(right) {}
    ...
    public:
        ...
        sub_script_expr(const char *file, const int line, const token *where, const expr *name, const expr *key) : expr(
                line, file, SUB_SCRIPT_EXPR_T), where(where), name(name), key(key) {}
    ...
    public:
        ...
        sub_script_assign_expr(const char *file, const int line, const token *where, const expr *name,
                               const token_type op, const expr *key, const expr *value) : expr(line, file,
                                                                                               SUB_SCRIPT_ASSIGN_EXPR_T),
    ...
    public:
        ...
        member_expr(const char *file, const int line, const expr *container_name, const token *member_identifier)
                : expr(line, file, MEMBER_EXPR_T), container_name(container_name),
    ...
    public:
        ...
        member_assign_expr(const char *file, const int line, const expr *container_name, const token_type op,
                           const token *member_identifier, const expr *value) : expr(line, file, MEMBER_ASSIGN_EXPR_T),
                                                                                container_name(container_name), op(op),
                                                                                member_identifier(member_identifier),
                                                                                value(value) {}
    ...
    public:
        ...
        array_expr(const char *file, const int line, const token *open_brace, const vector<pair<expr *, expr *>> &pairs)
                : expr(line, file, ARRAY_EXPR_T), open_brace(open_brace), pairs(pairs) {}

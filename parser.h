...
namespace lns {
    ...
    private:
        ...
        stmt* use() {
            ...
            string source;
            stringstream ss;
            if(!file.is_open()){ error(previous(),"use: file was not found or is unaccessible"); return nullptr;}
            ss << file.rdbuf();
            source = ss.str();
    ...
    public:
        explicit parser(vector<token> &tokens) : tokens(tokens), start(0), current(0), use_allowed(true), statements(*new vector<stmt*>()) {}
        parser() = delete;
        vector <stmt*>& parse() {
            stmt* s;
            while (!is_at_end()) {
